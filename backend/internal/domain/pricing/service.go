...
func NewPricingService(channels channel.Repository) *PricingService {
	return &PricingService{channels: channels}
}

// CalculateForChannel resolves the sales channel by code before running the
// same calculation as Calculate. It's the entry point controllers should use
// so channel lookup — an infrastructure concern — stays out of the HTTP layer.
func (s *PricingService) CalculateForChannel(ctx context.Context, input PricingInput) (PricingResult, error) {
	input.ChannelCode = strings.TrimSpace(input.ChannelCode)
	if input.ChannelCode == "" {
		return PricingResult{}, shared.ErrInvalidInput
	}
	salesChannel, err := s.channels.FindByCode(ctx, input.ChannelCode)
	if err != nil {
		return PricingResult{}, err
	}
	return s.Calculate(input, salesChannel)
}

func (s *PricingService) Calculate(input PricingInput, salesChannel channel.Channel) (PricingResult, error) {
	...
	switch input.Mode {
	case ModeAnalyzeSalePrice:
		if input.SalePriceCents == nil || *input.SalePriceCents <= 0 {
			return PricingResult{}, shared.ErrInvalidInput
		}
		return s.calculateAtPrice(input, salesChannel, *input.SalePriceCents)
	case ModeTargetMargin:
		if input.DesiredMarginBPS == nil || *input.DesiredMarginBPS < 0 || *input.DesiredMarginBPS >= bpsBase {
			return PricingResult{}, shared.ErrInvalidInput
		}
		return s.calculateTargetMargin(input, salesChannel, *input.DesiredMarginBPS)
	default:
		return PricingResult{}, shared.ErrInvalidInput
	}
}
...
func (s *PricingService) calculateTargetMargin(input PricingInput, salesChannel channel.Channel, desiredMarginBPS int64) (PricingResult, error) {
	low := s.minimumCostFloor(input, salesChannel)
	if low < 1 {
		low = 1
	}
	high := low * 3
	...
	for high <= maxSearchPrice {
		result, err := s.calculateAtPrice(input, salesChannel, high)
		if err != nil {
			return PricingResult{}, err
		}
	...
	if !found {
		return PricingResult{}, shared.ErrImpossibleMargin
	}

	for i := 0; i < 80 && low < high; i++ {
		mid := low + (high-low)/2
		result, err := s.calculateAtPrice(input, salesChannel, mid)
		if err != nil {
			return PricingResult{}, err
		}
		if result.MarginBPS >= desiredMarginBPS && result.NetProfitCents >= 0 {
	...
	best, err := s.calculateAtPrice(input, salesChannel, high)
	if err != nil {
		return PricingResult{}, err
	}
	...
	for attempts := 0; attempts < 10_000 && best.SalePriceCents > 1; attempts++ {
		previous, err := s.calculateAtPrice(input, salesChannel, best.SalePriceCents-1)
...
func (s *PricingService) calculateAtPrice(input PricingInput, salesChannel channel.Channel, salePriceCents int64) (PricingResult, error) {
	if salePriceCents <= 0 {
		return PricingResult{}, shared.ErrInvalidInput
	}
	...
	channelFee := s.calculateChannelFee(salesChannel, feeBaseCents, input.ChannelOptions)
...
func (s *PricingService) minimumCostFloor(input PricingInput, salesChannel channel.Channel) int64 {
	...
	channelFee := s.calculateChannelFee(salesChannel, 0, input.ChannelOptions)
	return input.ProductCostCents + manualFixedCents + logisticFixedCents + channelFee.fixedFeeCents + channelFee.optionFeeCents
}

func (s *PricingService) calculateChannelFee(salesChannel channel.Channel, feeBaseCents int64, options ChannelOptions) channelFeeResult {
	commissionBPS := salesChannel.FeeRules.DefaultCommissionBPS
	fixedFeeCents := salesChannel.FeeRules.FixedFeeCents
	label := "Comissão do canal"

	switch salesChannel.FeeRules.Strategy {
	case channel.FeeStrategyTiered:
		for _, tier := range salesChannel.FeeRules.Tiers {
	...
	case channel.FeeStrategyCategory:
		selected := strings.TrimSpace(options.CategoryCode)
		foundCategory := false
		for _, category := range salesChannel.FeeRules.Categories {
	...
	if salesChannel.FeeRules.MinCommissionCents > 0 && commissionCents < salesChannel.FeeRules.MinCommissionCents {
		commissionCents = salesChannel.FeeRules.MinCommissionCents
	}
	...
	for _, option := range salesChannel.FeeRules.Options {
